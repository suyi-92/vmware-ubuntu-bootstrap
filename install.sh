...
remote_bootstrap() {
  ...
  command -v git >/dev/null 2>&1 || remote_packages+=(git)
  command -v curl >/dev/null 2>&1 || remote_packages+=(curl)
  command -v python3 >/dev/null 2>&1 || remote_packages+=(python3)
  command -v ip >/dev/null 2>&1 || remote_packages+=(iproute2)
  [[ -s /etc/ssl/certs/ca-certificates.crt ]] || remote_packages+=(ca-certificates)

  if (( ${#remote_packages[@]} > 0 )); then
    command -v apt-get >/dev/null 2>&1 || {
      printf '缺少 apt-get，无法安装远程启动依赖。\n' >&2
      exit 1
    }
    printf '正在安装缺失的远程启动依赖：%s\n' "${remote_packages[*]}"
    sudo --preserve-env=http_proxy,https_proxy,HTTP_PROXY,HTTPS_PROXY,all_proxy,ALL_PROXY,no_proxy,NO_PROXY \
      env DEBIAN_FRONTEND=noninteractive apt-get update
    sudo --preserve-env=http_proxy,https_proxy,HTTP_PROXY,HTTPS_PROXY,all_proxy,ALL_PROXY,no_proxy,NO_PROXY \
      env DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends "${remote_packages[@]}"
  fi
...
collect_config() {
  ...
  detected_iface="$(current_interface 2>/dev/null || true)"
  ...
  detected_gateway="$(current_gateway "$NETWORK_INTERFACE")"
  ...
  PROXY_SCAN_CIDR="$(read_default "代理扫描网段" "${PROXY_SCAN_CIDR:-$(default_proxy_scan_cidr "$NETWORK_INTERFACE")}")"

  ui_section "固定网络"
  CONFIGURE_STATIC_NETWORK="$(bool_prompt "配置固定桥接 IP" "$CONFIGURE_STATIC_NETWORK")"
  show_network_state
  if is_true "$CONFIGURE_STATIC_NETWORK"; then
    STATIC_IPV4_CIDR="$(read_default "完整静态 IPv4/CIDR（必须由管理员分配，无默认地址）" "$STATIC_IPV4_CIDR")"
    STATIC_IPV4_PREFIX=""; STATIC_IPV4_LAST_OCTET=""; PREFIX_LENGTH=""
...
banner
dependency_args=(--phase dependencies --config "$VUB_CONFIG_FILE")
is_true "$VUB_DRY_RUN" && dependency_args+=(--dry-run)
is_true "$VUB_VERBOSE" && dependency_args+=(--verbose)
bash "$PROJECT_DIR/bootstrap.sh" "${dependency_args[@]}"
