...
实际刷新在代理配置成功后，独立验证本机对 GitHub、Ubuntu 的解析。使用 systemd-resolved 时也会执行不使用缓存的 DNS 查询；代理能够联网或命中旧缓存都不能替代该项检查。DNS 失败会返回非零状态，不显示“刷新完成”，已验证的新代理保留。预览不执行最终验收。如果物理网卡 DNS 正常而默认查询失败，输出会指出携带 `~.` 的接口；脚本不会擅自清空其他 VPN 的 DNS 或重启其服务。应在所属 VPN/TUN 服务中修正策略，再重跑刷新。
...
桌面配置只改动代理相关的键，同时将八个大小写代理变量同步到已有 D-Bus 和 systemd 用户会话，避免新启动的用户服务继承旧宿主地址。每次修改记录原值，`proxy-off` 和代理阶段回滚会恢复它们；外部修改的桌面设置或用户会话代理不会被关闭流程覆盖。D-Bus 没有删除环境变量的接口，原来未设置的代理会恢复为空值；systemd 用户环境会恢复为未设置。若当前用户尚未登录图形桌面，脚本会明确提示桌面代理未更新，登录后重跑即可。`source` 只刷新当前终端，不能代替桌面代理设置。

会话同步不会改写已经运行的 GNOME Shell、浏览器或终端进程的环境；直接从旧桌面进程派生的程序仍可能带入旧地址。脚本不会强制退出应用或注销桌面。请保存页面并从 Firefox 菜单完全退出，然后在 Ubuntu 终端执行：
